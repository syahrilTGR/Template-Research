...
## Reading Compressed Diagrams

Standard `.drawio` files may contain compressed XML data within the `<diagram>` tag. To read and analyze these files, use the following Python snippet to decode the content:

```python
import base64, zlib, urllib.parse, re

def decode_drawio(file_path):
    with open(file_path, 'r', encoding='utf-8') as f:
        content = f.read()
    
    # Extract the compressed data from the <diagram> tag
    ...
    # Decode: Base64 -> Decompress (Zlib) -> URL Unquote
    try:
...
# Example usage:
# print(decode_drawio('path/to/diagram.drawio'))
```

Always use this method before analyzing existing `.drawio` files to ensure you are seeing the actual XML structure.
