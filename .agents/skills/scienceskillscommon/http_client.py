...
import email.utils
import sys
# Windows compatibility: fcntl is Unix-only; use msvcrt + threading on Windows
...
class _RateLimiter:
  ...
  def wait(self, min_sleep: float = 0.0):
    ...
    """
    if sys.platform == 'win32':
      # Windows: use in-process threading lock (single-process rate limiting)
      with _win_lock:
        now = time.monotonic()
        gap = self._min_interval - (now - getattr(self, '_last_request_ts', 0.0))
